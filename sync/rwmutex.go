...
/*
	...
	TODO: 具体情况根据不同的机器和go版本而定, 以及基准测试情况; 以下结论来自于
	<go语言精进之路: 从新手到高手的编程思想, 方法和技巧1>
	在并发量较小的情况下, 互斥锁性能更好; 随着并发量增大, 互斥锁的竞争激烈,
	导致加锁和解锁性能下降; 读写锁的读锁性能不会随并发量的增大而发生较大变化,
	在并发量较大的情况下, 读写锁的写锁性能比互斥锁、读写锁的读锁都差, 并且随
	着并发量增大, 其写锁性能有继续下降的趋势;
	因此读写锁适合应用在具有一定并发量且读多写少的场合;
