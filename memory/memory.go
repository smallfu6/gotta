...
/*
	虚拟内存布局: TODO
	go1.0 以前的版本中, 堆区的内存空间都是连续的; 但是 go1.11 开始使用稀疏
	的堆内存空间替代了连续内存, 解决了连续内存带来的限制以及在特殊场景下可能
	遇到的问题;
	TODO: 了解线性内存

	稀疏内存:
	使用稀疏布局不仅能够移除堆大小的上限, 还能解决c语言和go语言混合使用时
	的地址空间冲突问题(TODO); 但因为基于稀疏内存的内存管理失去了内存连续
	性假设, 使得内存管理变得更加复杂;

	运行时使用二维 runtime.heapArea 数组管理所有内存, 每个单元都会管理
	64MB的内存空间;
*/
